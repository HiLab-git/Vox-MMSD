...
class MaskedImage(AbstractTransform):
    ...
    def __call__(self, sample):
        ...
        p_c, p_h, p_w = c//self.size, h // self.size, w // self.size
        mask = np.random.rand(1, p_c, p_h, p_w)
        mask = zoom(mask.reshape(1, p_c, p_h, p_w),
                    (1, self.size, self.size, self.size),
                    order=0) > (1 - self.ratio)
        mask = np.repeat(mask, b, 0)

        pad_c = c - mask.shape[1]
        pad_h = h - mask.shape[2]
        pad_w = w - mask.shape[3]
        mask = np.pad(mask, ((0, 0), (pad_c // 2, pad_c - pad_c // 2), (pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2)),
                    mode="constant")

        if self.method == 'zero':
            image[mask] = 0.0
        elif self.method == 'mean':
            image[mask] = image.mean()
        sample['image'] = image

        return sample

class ModMaskedImage(AbstractTransform):
    """
    Masked image modeling referring to the MAE, used for self-supervised learning 
    """
    def __init__(self, params):
        super(ModMaskedImage, self).__init__(params)
        self.ratio = params.get('Masked_ratio'.lower())
        self.prob  = params.get('Masked_prob'.lower())
        self.method = params.get('Masked_method'.lower())
    ...
    def __call__(self, sample):
        ...
        mask = np.random.rand(4,1,1,1)>(1-self.ratio)
        mask = zoom(mask, (1,d,h,w), order=0)
...
class VoxMMSDMaskedImage(AbstractTransform):
    """
    Masked image modeling referring to the MAE, used for self-supervised learning 
    """
    def __init__(self, params):
        super(VoxMMSDMaskedImage, self).__init__(params)
        self.ratio = params.get('Masked_ratio'.lower())
        self.size  = params.get('Masked_size'.lower())
        self.prob  = params.get('Masked_prob'.lower())
        self.method = params.get('Masked_method'.lower())
    ...
    def __call__(self, sample):
        ...
        b, c, h, w = image.shape
        p_c, p_h, p_w = c//self.size, h // self.size, w // self.size
        mask = np.random.rand(b, p_c, p_h, p_w)
        mask = zoom(mask.reshape(b, p_c, p_h, p_w),
                    (1, self.size, self.size, self.size),
                    order=0) > (1 - self.ratio)
...
class GaussianBlur(AbstractTransform):
    def __init__(self, params):
        ...
        # different sigma for per channel or not
        self.per_channel  = params.get('GaussianBlur_per_channel'.lower(), False)
        self.per_channel_prob = params.get('GaussianBlur_per_channel_prob'.lower(), 0.5)
    ...
    def __call__(self, sample):
        if(np.random.uniform() < self.prob):
            ...
            for chn in self.channels:
                if(np.random.uniform() < self.per_channel_prob):
                    if(self.per_channel):
                        sigma = random.uniform(self.sigma_min,self.sigma_max)
                    image[chn] = gaussian_filter(image[chn], sigma, order=0)
...
class BrightnessAdditive(AbstractTransform):
    def __init__(self, params):
        ...
        # different sigma for per channel or not
        self.per_channel  = params.get('BrightnessAdditive_per_channel'.lower(), False)
        self.per_channel_prob = params.get('BrightnessAdditive_per_channel_prob'.lower(), 0.5)
...
class BrightnessMultiplicative(AbstractTransform):
    def __init__(self, params):
        ...
        # different sigma for per channel or not
        self.per_channel  = params.get('BrightnessMultiplicative_per_channel'.lower(), False)
        self.per_channel_prob  = params.get('BrightnessMultiplicative_per_channel_prob'.lower(), 0.5)
    ...
    def __call__(self, sample):
        ...
        # if(sample['image'].max()==0):
        #     print('BrightnessMultiplicative')
        return sample
    
class Contrast(AbstractTransform):
    def __init__(self, params):
        super(Contrast, self).__init__(params)
        # self.channels = params.get('Contrast_channels'.lower(), None)
        # self.sigma_min     = params['Contrast_sigma_min'.lower()]
        # self.sigma_max     = params['Contrast_sigma_max'.lower()]
        self.prob     = params.get('Contrast_probability'.lower(), 0.5)
        # # different sigma for per channel or not
        # self.per_channel  = params.get('Contrast_per_channel'.lower(), False)
        # self.per_channel_prob  = params.get('Contrast_per_channel_prob'.lower(), 0.5)
    ...
    def __call__(self, sample):
        ...
        image = sample['image']
        img_aug = augment_contrast(image)
