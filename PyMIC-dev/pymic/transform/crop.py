...
class RandomResizedCrop(CenterCrop):
    ...
    def __init__(self, params):
        ...
        #在边缘减去margin的范围内进行裁剪，避免剪到全为0的值
        self.crop_margin_limit    = params.get('RandomResizedCrop_margin_limit'.lower(), None)
        self.task        = params['Task'.lower()]
        assert isinstance(self.output_size, (list, tuple))
    ...
    def __call__(self, sample):
        ...
        if(self.crop_margin_limit!=None):
            crop_margin = [input_size[i] - crop_size[i] - self.crop_margin_limit[i] for i in range(input_dim)]
        else:
        ...
        if(random.random() < self.fg_ratio):
            ...
            if(self.crop_margin_limit!=None):
                # print("input_size:{}, crop_margin:{}".format(input_size,crop_margin))
                crop_min = [random.randint(self.crop_margin_limit[i], crop_margin[i]) for i in range(len(crop_margin))]
            else:
        ...
        # if(sample['image'].max()==sample['image'].min()):
