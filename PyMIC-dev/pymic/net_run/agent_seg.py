...
class SegmentationAgent(NetRunAgent):
    ...
    def train_valid(self):
        ...
        if(ckpt_init_name is not None):
            checkpoint = torch.load(ckpt_init_name, map_location = self.device)
            pretrained_dict = checkpoint['model_state_dict']
            assert(self.config['training'].get('init_part', None)!=None)
            #根据init_part判断需要加载哪些权重
            if('header' not in self.config['training'].get('init_part')):
                for param in list(pretrained_dict.keys()):
                    if 'out' in param:
                        del pretrained_dict[param]
            if('decoder' not in self.config['training'].get('init_part')):
                for param in list(pretrained_dict.keys()):
                    if 'decoder' in param:
                        del pretrained_dict[param]
            ...
            for param in list(pretrained_dict.keys()):
                    if 'backbone' in param:
                        pretrained_dict[param[9:]]=pretrained_dict[param]
                        del pretrained_dict[param]
    ...
    def infer(self):
        ...
        if('model_state_dict' in checkpoint):
            ...
            #为了加载nnUNet训练得到的网络
            self.net.load_state_dict(checkpoint['network_weights'])
