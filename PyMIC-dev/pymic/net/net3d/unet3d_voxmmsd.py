...
class Decoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.same_pred  = self.params.get('same_pred', False)
        assert(len(self.ft_chns) == 5 or len(self.ft_chns) == 4)
        ...
        if(self.same_pred):
            self.out_conv1 = nn.Conv3d(self.ft_chns[1], self.ft_chns[0], kernel_size = 1)
            self.out_conv2 = nn.Conv3d(self.ft_chns[2], self.ft_chns[0], kernel_size = 1)
            self.out_conv3 = nn.Conv3d(self.ft_chns[3], self.ft_chns[0], kernel_size = 1)
        self.stage = 'train'
    ...
    def forward(self, x):
        ...
        #主要就是这里，不让网络用1*1卷积来把通道数改到和输入的通道数一致，而是直接输出feature
        output = [x_d0, x_d1, x_d2, x_d3]
        
        if(self.same_pred and self.stage == 'train'):
            output1 = self.out_conv1(x_d1)
            output2 = self.out_conv2(x_d2)
            output3 = self.out_conv3(x_d3)
            
            output = [x_d0, output1, output2, output3]


        return output

class UNet3D_voxmmsd(nn.Module):
    """
    ...
    * Reference: Özgün Çiçek, Ahmed Abdulkadir, Soeren S. Lienkamp, Thomas Brox, Olaf Ronneberger:
    ...
    """
    def __init__(self, params):
        super(UNet3D_voxmmsd, self).__init__()
