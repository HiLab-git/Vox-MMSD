...
class UNet3D_ccm(nn.Module):
    ...
    * Reference: Özgün Çiçek, Ahmed Abdulkadir, Soeren S. Lienkamp, Thomas Brox, Olaf Ronneberger:
    ...
    """
    def __init__(self, params):
        super(UNet3D_ccm, self).__init__()
        ...
        dim_mlp = params['feature_chns'][-1]

        self.avgpool = nn.AdaptiveAvgPool3d(1)
        self.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), nn.Linear(dim_mlp, 128))
    ...
    def forward(self, x):
        f = self.encoder(x)
        avg_f = self.avgpool(f[4])
        flat_f=torch.flatten(avg_f,1)
        feture = self.fc(flat_f)
        rec = self.decoder(f)
